...
(define i-match?
  (lambda (state symbol inst)
    (and (equal? state (i-state inst)) (equal? symbol (i-symbol inst)))))
    

(define i-find
  (lambda (state symbol inst)
    (cond ((null? inst) #f)
          ((i-match? state symbol (first inst)) (first inst))
          (else (i-find state symbol (rest inst))))))
...
(define c-symbol
  (lambda (config)
    (cond ((equal? (c-head-position config) 0) (first (c-tape config)))
          (else (c-symbol (list (c-state config) (- (c-head-position config) 1) (rest (c-tape config))))))))
...
(define halted?
  (lambda (mach config)
    (cond ((null? mach) #t)
          ((i-match? (c-state config) (c-symbol config) (first mach)) #f)
          (else (halted? (rest mach) config)) )))
...
(define change-tape1
  (lambda (s config)
    (cond ((equal? (c-head-position config) 0) (cons s (rest (c-tape config))))
          (else (cons (first (c-tape config)) (change-tape1 s (list (c-state config) (- (c-head-position config) 1) (rest (c-tape config)))))))))

(define write-symbol
  (lambda (s config)
    (list (c-state config) (c-head-position config) (change-tape1 s config))))
...
(define move-head
  (lambda (dir config)
    (cond ((equal? dir 'R) (cond  ((equal? (+ (c-head-position config) 1) (length (c-tape config))) (list (c-state config) (+ (c-head-position config) 1) (append (c-tape config) '(b))))
                                  (else (list (c-state config) (+ (c-head-position config) 1) (c-tape config)))))
          (else (cond ((equal? (- (c-head-position config) 1) -1) (list (c-state config) 0 (cons 'b (c-tape config))))
                                  (else (list (c-state config) (- (c-head-position config) 1) (c-tape config))))))))
...
(define next-config
  (lambda (tm1 config)
    (cond  ((halted? tm1 config) config)
           (else (do-something-else tm1 config)))))

(define do-something-else
  (lambda (tm1 config)
    (let ((y (i-find  (c-state config) (c-symbol config) tm1)))
      (change-state (i-new-state y) (move-head (i-direction y) (write-symbol (i-new-symbol y) config))))))
...
(define mid1 (list'(q1 c q1 c R)'(q1 b q2 1 L)'(q2 c q2 c L)'(q2 b q3 2 R)))
